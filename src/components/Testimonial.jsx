...
const Testimonial = () => {
  ...
  return (
    <div className="py-28 px-6 md:px-16 lg:px-24 xl:px-44">
      <Title title="What Our Customers Say" subTitle="Discover why discerning travelers choose CarRental for their car rental around the world." />
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 mt-18">
        {testimonials.map((testimonial, index) => (
            ...
            <div className="flex items-center gap-1 mt-4">
                {Array(5).fill(0).fill(1, 0, testimonial.rating).map((item, index) => (
                  <img key={index} src={item? assets.star_icon : assets.star_outline_icon} alt="star icon" />
                ))}
            </div>
